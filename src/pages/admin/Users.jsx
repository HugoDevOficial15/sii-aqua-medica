...
export default function Users({ onClose }) {
  ...
  const isWoman = (user) => {
    const genero = String(user?.Genero || user?.genero || "")
      .trim()
      .toUpperCase();
  ...
  const handleFindDuplicates = async () => {
    try {
      ...
      if (duplicates.length === 0) {
        notifySuccess("Sin duplicados", "No se encontraron nóminas repetidas.");
        return;
      }
      ...
      notifyError(
        "Error",
        "No se pudo completar el diagnóstico de duplicados.",
      );
    }
  };
  ...
  const handleCheckEmailNominaMismatch = async () => {
    try {
      ...
      if (mismatches.length === 0) {
        notifySuccess(
          "Sin problemas",
          "Todos los emails coinciden con sus nóminas.",
        );
        return;
      }
  ...
  const handleSaveIncapacidad = async (event) => {
    ...
    if (
      new Date(incapacidadForm.fechaFin) < new Date(incapacidadForm.fechaInicio)
    ) {
      notifyError(
        "Fechas inválidas",
          ...
          <table className="table custom-table">
            ...
            <tbody>
              {currentUsers.map((user) => {
                const isExpanded = expandedUserId === user.id;
                const incapacidades = Array.isArray(userIncapacidades[user.id])
                  ? userIncapacidades[user.id]
                  : [];
                const isValidatingIncapacidades =
                  !!loadingIncapacidades[user.id];
                const hasLoadedIncapacidades =
                ...
                return (
                  <Fragment key={user.id}>
                    <tr
                      ...
                      onClick={(event) => {
                        if (
                          event.target.closest(".users-actions-cell") ||
                          event.target.closest(".user-action-menu") ||
                          event.target.closest(".user-action-menu-button")
                        ) {
                          return;
                        }
                    ...
                    {isExpanded && (
                      <tr
                        key={`${user.id}-incapacidades`}
                        className="user-incapacidades-row"
                      >
              ...
              <div className="modal-body">
                <div className="row g-3">
                  ...
                  <div className="col-md-4">
                    <label>Nómina</label>
                    <input
                      className={`form-control ${errors.nomina ? "is-invalid" : ""}`}
                      {...register("nomina")}
                    />
                  </div>
