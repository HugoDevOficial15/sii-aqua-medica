...
const buildMaterialGroups = (stock = [], racks = [], planta = "") => {
  const rackMap = Object.fromEntries(racks.map((rack) => [rack.id, rack]));
  ...
  stock.forEach((item) => {
    ...
    group.lastEntry =
      compareDates(entryDate, group.lastEntry) < 0
        ? entryDate
  ...
  return Array.from(groups.values())
    .map((group) => ({
      ...group,
      racks: group.racks.sort((a, b) =>
        compareDates(a.ultimaEntrada, b.ultimaEntrada),
      ),
    }))
...
export default function AlmacenMaterialesPage() {
  ...
  const tiposDisponibles = useMemo(
    () =>
      [
        ...new Set(materiales.map((item) => item.tipoItem).filter(Boolean)),
      ].sort(),
    [materiales],
  );
  ...
  const filteredMateriales = useMemo(() => {
    ...
    return materiales.filter((item) => {
      const matchesSearch =
        !search || item.nombreItem.toLowerCase().includes(search);
      const matchesTipo = !filters.tipo || item.tipoItem === filters.tipo;
      const matchesMaterial =
        !filters.material || item.nombreItem === filters.material;
  ...
  const handleVerRack = (rack) => {
    ...
    navigate(
      `/almacen/peps?rackId=${encodeURIComponent(rack.rackId)}&rackNumero=${encodeURIComponent(rackNumero)}&planta=${encodeURIComponent(planta)}`,
    );
  };
    ...
    <div className="page-transition">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div className="page mb-3">
          <h6>
            <strong>Almacén Materiales</strong>
          </h6>
                            ...
                            <div className="rack-location-list">
                              {item.racks.map((rack) => (
                                ...
                                >
                                  <thead>
                                    <tr>
                                      <th className="rack-location-head">
                                        {rack.ubicacionLabel}
                                      </th>
                                      <th className="rack-location-head">
                                        Planta: {rack.planta}
                                      ...
                                      <th className="rack-location-head">
                                        Cantidad: {rack.cantidad}
                                      </th>
                                      <th className="rack-location-head">
                                        Última entrada:{" "}
                                        {formatDate(rack.ultimaEntrada)}
                                      </th>
